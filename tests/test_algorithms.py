...
def test_custom_palette_loader(tmp_path: Path) -> None:
    ...
    # Setting a flattened palette should be sanitised
    processor.set_custom_palette(np.array([10, 20, 30, 40, 50, 60], dtype=np.uint8))
    assert processor._custom_palette is not None  # type: ignore[attr-defined]
