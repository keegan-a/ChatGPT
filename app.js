...
const diagnosticsVersion = document.querySelector('#diagnosticsVersion');
const diagnosticsSw = document.querySelector('#diagnosticsSw');
const diagnosticsCaches = document.querySelector('#diagnosticsCaches');
const diagnosticsUpdated = document.querySelector('#diagnosticsUpdated');
const diagnosticsFootnote = document.querySelector('#diagnosticsFootnote');
const diagnosticsRefreshBtn = document.querySelector('#diagnosticsRefresh');
...
let lastDiagnosticsRun = null;
let serviceWorkerRegistration = null;

if (diagnosticsVersion) {
  diagnosticsVersion.textContent = `v${APP_VERSION}`;
}
if (diagnosticsSw) {
  diagnosticsSw.textContent = 'Run report';
}
if (diagnosticsCaches) {
  diagnosticsCaches.textContent = 'Run report';
}
...
function registerWindows() {
  ...
  panels.forEach((panel) => {
    ...
    if (!panel.dataset.homeState) {
      panel.dataset.homeState = initialState;
    }
    if (!panel.dataset.homeX) {
      panel.dataset.homeX = panel.getAttribute('data-x') || panel.dataset.x || '0';
    }
    if (!panel.dataset.homeY) {
      panel.dataset.homeY = panel.getAttribute('data-y') || panel.dataset.y || '0';
    }
    if (!panel.dataset.homeWidth) {
      panel.dataset.homeWidth = panel.getAttribute('data-width') || panel.dataset.width || String(panel.offsetWidth || 0);
    }
    if (!panel.dataset.homeHeight) {
      panel.dataset.homeHeight = panel.getAttribute('data-height') || panel.dataset.height || String(panel.offsetHeight || 0);
    }
...
function resetWindowLayout() {
  ...
  windowRegistry.forEach((record) => {
    const panel = record.element;
    if (!panel) return;
    const homeX = Number.parseFloat(panel.dataset.homeX || '0');
    const homeY = Number.parseFloat(panel.dataset.homeY || '0');
    const homeWidth = Number.parseFloat(panel.dataset.homeWidth || `${panel.offsetWidth || frameRect.width / 2}`);
    const homeHeight = Number.parseFloat(panel.dataset.homeHeight || `${panel.offsetHeight || frameRect.height / 2}`);
    const homeState = panel.dataset.homeState || 'open';
    record.restoreBounds = null;
    record.maximized = false;
    panel.dataset.maximized = 'false';
    panel.dataset.width = String(homeWidth);
    panel.dataset.height = String(homeHeight);
    panel.dataset.x = String(homeX);
    panel.dataset.y = String(homeY);
    if (windowModeActive) {
      panel.style.left = `${homeX}px`;
      panel.style.top = `${homeY}px`;
      panel.style.width = `${homeWidth}px`;
      panel.style.height = `${homeHeight}px`;
      constrainPanelToFrame(panel, frameRect);
    } else {
...
function announceLayoutReset() {
  const message = `Window layout reset at ${formatTimestamp(new Date())}.`;
  if (diagnosticsFootnote) {
    diagnosticsFootnote.innerHTML = `<strong>${message}</strong> Run diagnostics or refresh caches from the Start menu if something still looks off.`;
  }
...
async function gatherDiagnostics() {
    ...
    try {
      ...
      if (registration) {
        ...
        if (registration.waiting) {
          swStatus = `Waiting (${registration.waiting.state})`;
        } else if (registration.installing) {
          swStatus = `Installing (${registration.installing.state})`;
      ...
      swStatus = `Error: ${error instanceof Error ? error.message : String(error)}`;
    }
  }
  ...
  if (window.caches) {
    try {
      ...
      cacheKeys = [`Error: ${error instanceof Error ? error.message : String(error)}`];
    }
  } else {
...
function renderDiagnosticsPanel(result) {
  ...
  if (diagnosticsSw) {
    diagnosticsSw.textContent = result.swScope ? `${result.swStatus} @ ${result.swScope}` : result.swStatus;
  }
  if (diagnosticsCaches) {
    diagnosticsCaches.textContent = result.cacheKeys.length
      ? result.cacheKeys.join(', ')
      : 'None';
  }
  if (diagnosticsUpdated) {
    diagnosticsUpdated.textContent = formatTimestamp(result.timestamp);
  }
  if (diagnosticsFootnote) {
    diagnosticsFootnote.innerHTML =
      'Tip: Use <strong>Refresh & Clear Cache</strong> in the Start menu to force-install the newest build if the interface looks outdated.';
  }
}
...
async function runDiagnostics() {
  ...
  if (diagnosticsSw) {
    diagnosticsSw.textContent = 'Checking…';
  }
  if (diagnosticsCaches) {
    diagnosticsCaches.textContent = 'Checking…';
  }
  if (diagnosticsUpdated) {
    diagnosticsUpdated.textContent = 'Working…';
  }
  try {
    const result = await gatherDiagnostics();
    lastDiagnosticsRun = result.timestamp;
    renderDiagnosticsPanel(result);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    if (diagnosticsFootnote) {
      diagnosticsFootnote.textContent = `Diagnostics failed: ${message}`;
    }
    console.error('Diagnostics failed:', error);
  }
}

async function refreshAndReloadApplication() {
  closeStartMenu();
  alert('Clearing cached files and refreshing the desktop. The page will reload when finished.');
  try {
    if ('serviceWorker' in navigator) {
...
if (startMenu) {
  startMenu.addEventListener('click', (event) => {
    ...
    const resetLayout = button.hasAttribute('data-reset-layout');
    const refreshApp = button.hasAttribute('data-refresh-app');
    if (windowId) {
      openWindow(windowId);
      if (windowId === 'snake') {
        drawSnakeFrame();
      }
      if (windowId === 'diagnostics') {
        runDiagnostics();
      }
      closeStartMenu();
    } else if (theme) {
...
if ('serviceWorker' in navigator) {
  ...
  navigator.serviceWorker.addEventListener('controllerchange', () => {
    if (diagnosticsFootnote) {
      diagnosticsFootnote.textContent = 'Update installed. Reloading…';
    }
    window.location.reload();
  });
}
