...
class ImageProcessor(QObject):
    ...
    def _apply_post_processing(
        ...
        if request.glow_radius > 0:
            radius = request.glow_radius
            reference_img = Image.fromarray(toned_reference.astype(np.uint8))
            blur = reference_img.filter(ImageFilter.GaussianBlur(radius))
            blur_np = np.asarray(blur, dtype=np.float32)
            halo = np.clip(blur_np - toned_reference, 0.0, 255.0)
            strength = min(2.5, 0.25 + radius / 18.0)
            result = np.clip(result + halo * strength, 0, 255)
