...
class ImageProcessor(QObject):
    ...
    def _apply_post_processing(
        ...
        if request.glow_radius > 0:
            glow_source = toned_reference.astype(np.float32)
            luminance = (
                glow_source[..., 0] * 0.299
                + glow_source[..., 1] * 0.587
                + glow_source[..., 2] * 0.114
            )
            highlight = np.clip((luminance - 160.0) / 95.0, 0.0, 1.0)
            highlight = highlight**1.2
            glow_rgb = np.clip(glow_source * highlight[..., None], 0.0, 255.0).astype(np.uint8)
            pil_glow = Image.fromarray(glow_rgb).filter(ImageFilter.GaussianBlur(request.glow_radius))
            intensity = min(request.glow_radius, 50) / 35.0
            result = np.clip(result + np.asarray(pil_glow, dtype=np.float32) * intensity, 0, 255)
