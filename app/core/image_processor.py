...
class ImageProcessor(QObject):
    ...
    def _apply_pixel_size(
        self, image: Image.Image, pixel_size: float
    ) -> tuple[Image.Image, tuple[int, int] | None]:
        pixel_size = max(1.0, float(pixel_size))
        image = image.convert("RGB")
        if math.isclose(pixel_size, 1.0, rel_tol=0.0, abs_tol=1e-3):
            return image, None

        reduced_size = (
            max(1, int(round(image.width / pixel_size))),
            max(1, int(round(image.height / pixel_size))),
        )
    ...
    def _apply_glow(
        self, image: np.ndarray, reference: np.ndarray, radius: int
    ) -> np.ndarray:
        radius = max(radius, 1)
        normalised = np.clip(reference / 255.0, 0.0, 1.0)
        luminance = np.dot(normalised, np.array([0.2126, 0.7152, 0.0722], dtype=np.float32))
        glow_seed = np.clip((luminance - 0.55) * 4.0, 0.0, 1.0)
        glow_img = Image.fromarray((glow_seed * 255.0).astype(np.uint8))
        blurred = glow_img.filter(ImageFilter.GaussianBlur(radius))
        halo = np.asarray(blurred, dtype=np.float32)[..., None] / 255.0
        glow_strength = min(radius / 20.0, 2.0)
        boosted = image + halo * reference * glow_strength
        return np.clip(boosted, 0.0, 255.0).astype(np.float32)
