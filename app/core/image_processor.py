...
class ImageProcessor:
    ...
    def set_image(self, image: Image.Image) -> None:
        ...
        image.load()  # ensure file handles are released before background work
    ...
    def set_custom_palette(self, palette: PaletteArray | None) -> None:
        ...
        if array.ndim == 1:
            if array.size % 3:
                raise ValueError("Palette array must be divisible into RGB triplets")
            array = array.reshape(-1, 3)

        if array.shape[-1] not in (3, 4):
            raise ValueError("Palette array must have 3 (RGB) or 4 (RGBA) columns")
    ...
    def _apply_adjustments(self, rgb: np.ndarray, settings: DitheringSettings) -> np.ndarray:
        ...
        if settings.sharpen_amount > 0:
            sigma = 1.0
            data = _safe_unsharp_mask(data, sigma, settings.sharpen_amount)
        if settings.denoise_strength > 0:
            data = _safe_denoise(data, settings.denoise_strength)
    ...
    def _build_palette(self, rgb: np.ndarray, settings: DitheringSettings) -> np.ndarray:
        ...
        if palette.size == 0:
            logger.warning("Palette construction returned no colours; falling back to grayscale")
            levels = np.linspace(0, 1, count)
            palette = np.stack([levels, levels, levels], axis=-1)
...
def _safe_gaussian_blur(image: np.ndarray, sigma: float) -> np.ndarray:
    """Apply Gaussian blur while tolerating OpenCV failures."""

    arr = np.ascontiguousarray(image.astype(np.float32, copy=False))
    try:
        return cv2.GaussianBlur(arr, ksize=(0, 0), sigmaX=sigma, sigmaY=sigma)
    except cv2.error as exc:  # pragma: no cover - depends on backend availability
        logger.warning("OpenCV GaussianBlur failed (%s); using numpy fallback", exc)
        radius = max(1, int(np.ceil(sigma * 2)))
        ...
        for c in range(arr.shape[-1]):
            tmp = np.apply_along_axis(lambda m: np.convolve(m, kernel, mode="valid"), 1, padded[:, :, c])
            padded_tmp = np.pad(tmp, ((radius, radius), (0, 0)), mode="edge")
            blurred[:, :, c] = np.apply_along_axis(lambda m: np.convolve(m, kernel, mode="valid"), 0, padded_tmp)
        return np.clip(blurred, 0.0, 1.0)


def _safe_unsharp_mask(image: np.ndarray, sigma: float, amount: float) -> np.ndarray:
    base = _safe_gaussian_blur(image, sigma)
    sharpened = image * (1.0 + amount) - base * amount
    return np.clip(sharpened, 0.0, 1.0)


def _safe_denoise(image: np.ndarray, strength: float) -> np.ndarray:
    temp = np.clip(image * 255.0, 0, 255).astype(np.uint8)
    h = 5 + int(strength * 10)
    try:
        ...
        logger.warning("OpenCV fastNlMeansDenoisingColored failed (%s); skipping denoise", exc)
