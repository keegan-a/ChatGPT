"""High level image processing pipeline for the dithering application."""

from __future__ import annotations

import threading
from dataclasses import dataclass
from pathlib import Path
from typing import List, Optional, Sequence, Tuple

try:  # pragma: no cover - OpenCV is optional at runtime
    import cv2  # type: ignore
except Exception:  # pragma: no cover - handled via graceful fallbacks
    cv2 = None  # type: ignore[assignment]

try:  # pragma: no cover - imageio is optional at runtime
    import imageio.v3 as imageio  # type: ignore
except Exception:  # pragma: no cover - handled via graceful fallbacks
    imageio = None  # type: ignore[assignment]

import numpy as np
from PIL import Image, ImageFilter

try:  # Pillow < 9 exposes resampling filters on the Image module directly
    RESAMPLING = Image.Resampling  # type: ignore[attr-defined]
except AttributeError:  # pragma: no cover - compatibility with older Pillow
    RESAMPLING = Image  # type: ignore[assignment]

try:  # pragma: no cover - optional dependency that may not be present
    from skimage import exposure  # type: ignore
except Exception:  # pragma: no cover - handled via fallback implementation
    exposure = None  # type: ignore[assignment]

from .dithering import get_algorithm

PaletteArray = np.ndarray


PALETTE_PRESETS: dict[str, List[Tuple[int, int, int]]] = {
    "GameBoy (4 colors)": [(15, 56, 15), (48, 98, 48), (139, 172, 15), (155, 188, 15)],
    "NES (16 colors)": [
        (124, 124, 124),
        (0, 0, 252),
        (0, 0, 188),
        (68, 40, 188),
        (148, 0, 132),
        (168, 0, 32),
        (168, 16, 0),
        (136, 20, 0),
        (80, 48, 0),
        (0, 120, 0),
        (0, 104, 0),
        (0, 88, 0),
        (0, 64, 88),
        (0, 0, 0),
        (188, 188, 188),
        (248, 248, 248),
    ],
    "CGA 16-color": [
        (0, 0, 0),
        (0, 0, 170),
        (0, 170, 0),
        (0, 170, 170),
        (170, 0, 0),
        (170, 0, 170),
        (170, 85, 0),
        (170, 170, 170),
        (85, 85, 85),
        (85, 85, 255),
        (85, 255, 85),
        (85, 255, 255),
        (255, 85, 85),
        (255, 85, 255),
        (255, 255, 85),
        (255, 255, 255),
    ],
}


def _sanitise_palette(palette: np.ndarray) -> PaletteArray:
    """Normalise palette input to unique ``uint8`` RGB triplets."""

    arr = np.asarray(palette)
    if arr.ndim == 0:
        raise ValueError("Custom palette must contain at least one colour")
    if arr.ndim == 1:
        usable = (arr.size // 3) * 3
        if usable == 0:
            raise ValueError("Custom palette must contain RGB triplets")
        arr = arr[:usable].reshape(-1, 3)
    elif arr.shape[-1] >= 3:
        arr = arr.reshape(-1, arr.shape[-1])[:, :3]
    else:
        raise ValueError("Custom palette must contain RGB triplets")

    if arr.size == 0:
        raise ValueError("Custom palette is empty")

    if np.issubdtype(arr.dtype, np.floating):
        max_val = float(arr.max()) if arr.size else 0.0
        if max_val <= 1.0 + 1e-6:
            arr = np.clip(np.rint(arr * 255.0), 0, 255)
        else:
            arr = np.clip(np.rint(arr), 0, 255)
    else:
        arr = np.clip(arr, 0, 255)

    unique = np.unique(arr.astype(np.uint8), axis=0)
    if unique.size == 0:
        raise ValueError("Custom palette is empty")
    return unique


@dataclass
class DitheringSettings:
    algorithm: str = "Floyd-Steinberg"
    serpentine: bool = True
    color_count: int = 8
    palette_mode: str = "Original Image Palette"
    custom_palette: Optional[PaletteArray] = None
    brightness: float = 0.0  # range [-1, 1]
    contrast: float = 0.0  # range [-1, 1]
    gamma: float = 1.0  # >0
    blur_radius: float = 0.0
    sharpen_amount: float = 0.0
    denoise_strength: float = 0.0
    preserve_brightness: bool = False
    transparent_background: bool = True


@dataclass
class ProcessResult:
    image: Image.Image
    palette: PaletteArray
    settings: DitheringSettings


class ImageProcessor:
    """Perform image adjustments, palette construction, and dithering."""

    def __init__(self) -> None:
        self._source_image: Optional[Image.Image] = None
        self._custom_palette: Optional[PaletteArray] = None
        self._lock = threading.RLock()

    # ------------------------------------------------------------------ general
    @property
    def has_image(self) -> bool:
        return self._source_image is not None

    def set_image(self, image: Image.Image) -> None:
        """Store the source image for subsequent renders."""

        # ``load`` eagerly reads the file so the worker threads do not rely on
        # an open file handle. Converting to RGBA guarantees consistent channel
        # counts for downstream processing and alpha preservation.
        image.load()
        if image.mode not in {"RGB", "RGBA"}:
            image = image.convert("RGBA")
        with self._lock:
            self._source_image = image.copy()

    # --------------------------------------------------------------- palettes --
    @property
    def available_palettes(self) -> List[str]:
        return ["Original Image Palette", "Grayscale", "Custom Palette"] + list(PALETTE_PRESETS.keys())

    def set_custom_palette(self, palette: PaletteArray | None) -> Optional[PaletteArray]:
        """Store a custom palette after sanitising its shape and dtype."""

        with self._lock:
            if palette is None:
                self._custom_palette = None
                return None
            sanitised = _sanitise_palette(np.asarray(palette))
            self._custom_palette = sanitised
            return sanitised

    def load_palette_from_file(self, path: Path) -> PaletteArray:
        """Load palette colours from an image or text file."""

        suffix = path.suffix.lower()
        if suffix in {".png", ".jpg", ".jpeg", ".bmp", ".gif"}:
            if imageio is not None:
                data = imageio.imread(path)
            else:
                with Image.open(path) as palette_image:
                    data = np.asarray(palette_image.convert("RGB"))
            data = np.asarray(data)
            if data.ndim == 3 and data.shape[-1] >= 3:
                base = data[..., :3]
            else:
                base = np.stack([data] * 3, axis=-1)
            base = np.clip(base, 0, 255).astype(np.uint8)
            colours = np.unique(base.reshape(-1, 3), axis=0)
        else:
            colours: List[Tuple[int, int, int]] = []
            for line in path.read_text().splitlines():
                line = line.strip()
                if not line or line.startswith("#"):
                    continue
                if line.startswith("0x") or line.startswith("#"):
                    token = line.lstrip("#")
                    if len(token) == 6:
                        colours.append(tuple(int(token[i : i + 2], 16) for i in range(0, 6, 2)))
                else:
                    parts = [p for p in line.replace(",", " ").split(" ") if p]
                    if len(parts) >= 3:
                        try:
                            colours.append(tuple(int(float(p)) for p in parts[:3]))
                        except ValueError:
                            continue
            if not colours:
                raise ValueError("No colours parsed from palette file")
            colours_np = np.array(colours, dtype=np.uint8)
            colours = np.unique(colours_np, axis=0)
        palette = _sanitise_palette(colours.astype(np.uint8))
        self.set_custom_palette(palette)
        return palette

    # --------------------------------------------------------------- rendering --
    def render_preview(self, settings: DitheringSettings, max_size: int = 640) -> ProcessResult:
        return self._render(settings, max_size=max_size)

    def render_full(self, settings: DitheringSettings) -> ProcessResult:
        return self._render(settings, max_size=None)

    def _render(self, settings: DitheringSettings, max_size: Optional[int]) -> ProcessResult:
        with self._lock:
            if self._source_image is None:
                raise RuntimeError("No image loaded")
            source_image = self._source_image.copy()
            custom_palette = None if self._custom_palette is None else self._custom_palette.copy()

        if max_size is not None:
            scale = min(1.0, max_size / max(source_image.width, source_image.height))
            if scale < 1.0:
                new_size = (
                    max(1, int(source_image.width * scale)),
                    max(1, int(source_image.height * scale)),
                )
                try:
                    resample_filter = RESAMPLING.LANCZOS  # Pillow ≥ 9
                except AttributeError:  # pragma: no cover - very old Pillow builds
                    resample_filter = getattr(RESAMPLING, "ANTIALIAS", getattr(RESAMPLING, "BICUBIC", Image.BICUBIC))
                working = source_image.resize(new_size, resample_filter)
            else:
                working = source_image
        else:
            working = source_image

        arr = np.array(working).astype(np.float32) / 255.0
        if arr.ndim == 2:
            arr = np.stack([arr] * 3, axis=-1)
        if arr.shape[-1] == 4:
            rgb = arr[..., :3]
            alpha = arr[..., 3:4]
        else:
            rgb = arr[..., :3]
            alpha = None

        adjusted = self._apply_adjustments(rgb, settings)
        palette = self._build_palette(adjusted, settings, custom_palette)

        if settings.preserve_brightness:
            adjusted_linear = _srgb_to_linear(adjusted)
            palette_linear = _srgb_to_linear(palette)
            dithered = self._apply_dither(adjusted_linear, palette_linear, settings)
            dithered = _linear_to_srgb(dithered)
        else:
            dithered = self._apply_dither(adjusted, palette, settings)

        if alpha is not None and settings.transparent_background:
            output = np.concatenate([dithered, alpha], axis=-1)
            mode = "RGBA"
        else:
            output = dithered
            mode = "RGB"

        image = Image.fromarray(_float_to_uint8(output), mode=mode)
        palette_display = _float_to_uint8(palette.reshape(-1, 3)).reshape(palette.shape)
        return ProcessResult(image=image, palette=palette_display, settings=settings)

    # ------------------------------------------------------------- adjustments --
    def _apply_adjustments(self, rgb: np.ndarray, settings: DitheringSettings) -> np.ndarray:
        data = rgb.copy()
        if settings.brightness:
            data = np.clip(data + settings.brightness, 0.0, 1.0)
        if settings.contrast:
            factor = 1.0 + settings.contrast
            data = np.clip((data - 0.5) * factor + 0.5, 0.0, 1.0)
        if settings.gamma and not np.isclose(settings.gamma, 1.0):
            data = np.clip(_adjust_gamma(data, settings.gamma), 0.0, 1.0)
        if settings.blur_radius > 0:
            data = _gaussian_blur(data, settings.blur_radius)
        if settings.sharpen_amount > 0:
            data = _unsharp_mask(data, settings.sharpen_amount)
        if settings.denoise_strength > 0:
            data = _denoise_image(data, settings.denoise_strength)
        return np.clip(data, 0.0, 1.0)

    # --------------------------------------------------------------- palette ---
    def _build_palette(
        self,
        rgb: np.ndarray,
        settings: DitheringSettings,
        custom_palette: Optional[PaletteArray],
    ) -> np.ndarray:
        mode = settings.palette_mode
        count = max(2, settings.color_count)
        if mode == "Original Image Palette":
            image = Image.fromarray(np.clip(rgb * 255.0, 0, 255).astype(np.uint8), mode="RGB")
            try:
                quantised = image.convert("RGB").quantize(colors=count, method=Image.MEDIANCUT, dither=Image.NONE)
                palette = np.array(quantised.convert("RGB"))
                palette = np.unique(palette.reshape(-1, 3), axis=0)
            except Exception:
                # Pillow can occasionally fail for large or low-variance images;
                # fall back to a simple grayscale ramp to keep the pipeline alive.
                levels = np.linspace(0, 255, count, dtype=np.float32)
                palette = np.stack([levels, levels, levels], axis=-1)
        elif mode == "Grayscale":
            levels = np.linspace(0, 1, count)
            palette = np.stack([levels, levels, levels], axis=-1)
        elif mode == "Custom Palette":
            if custom_palette is None:
                palette = np.linspace(0, 1, count)
                palette = np.stack([palette, palette, palette], axis=-1)
            else:
                palette = custom_palette.astype(np.float32) / 255.0
        elif mode in PALETTE_PRESETS:
            palette = np.array(PALETTE_PRESETS[mode], dtype=np.float32) / 255.0
        else:
            palette = np.linspace(0, 1, count)
            palette = np.stack([palette, palette, palette], axis=-1)

        palette = np.asarray(palette)
        if palette.ndim == 1:
            palette = np.tile(palette[:, None], (1, 3))
        palette = palette.reshape(-1, palette.shape[-1])
        if palette.shape[-1] > 3:
            palette = palette[:, :3]
        if palette.dtype != np.float32:
            palette = palette.astype(np.float32)
        if palette.max(initial=0.0) > 1.0:
            palette = palette / 255.0
        palette = np.clip(palette, 0.0, 1.0)
        if palette.shape[0] == 0:
            levels = np.linspace(0, 1, count, dtype=np.float32)
            palette = np.stack([levels, levels, levels], axis=-1)

        palette = palette.reshape(-1, 3)
        if palette.shape[0] > count:
            palette = palette[:count]
        elif palette.shape[0] < count:
            repeats = int(np.ceil(count / palette.shape[0]))
            palette = np.tile(palette, (repeats, 1))[:count]
        return palette.astype(np.float32)

    # --------------------------------------------------------------- dithering --
    def _apply_dither(self, rgb: np.ndarray, palette: np.ndarray, settings: DitheringSettings) -> np.ndarray:
        palette = np.ascontiguousarray(np.clip(palette, 0.0, 1.0).astype(np.float32))
        algorithm = get_algorithm(settings.algorithm)
        if algorithm.kind == "error_diffusion":
            return _error_diffusion(rgb, palette, algorithm.kernel, settings.serpentine)
        if algorithm.kind == "ordered":
            return _ordered_dither(rgb, palette, algorithm.matrix)
        return _random_dither(rgb, palette)


# -----------------------------------------------------------------------------
# Dithering helpers
# -----------------------------------------------------------------------------


def _nearest_palette_index(pixel: np.ndarray, palette: np.ndarray) -> int:
    distances = np.sum((palette - pixel) ** 2, axis=1)
    return int(np.argmin(distances))


def _error_diffusion(
    image: np.ndarray,
    palette: np.ndarray,
    kernel: Sequence[Tuple[int, int, float]],
    serpentine: bool,
) -> np.ndarray:
    working = image.copy()
    height, width, channels = working.shape
    output = np.zeros_like(working)
    for y in range(height):
        if serpentine and y % 2 == 1:
            x_range = range(width - 1, -1, -1)
            direction = -1
        else:
            x_range = range(width)
            direction = 1
        for x in x_range:
            old_pixel = working[y, x]
            idx = _nearest_palette_index(old_pixel, palette)
            new_pixel = palette[idx]
            output[y, x] = new_pixel
            error = old_pixel - new_pixel
            for dy, dx, weight in kernel:
                ny = y + dy
                nx = x + (dx * direction if direction == -1 else dx)
                if 0 <= ny < height and 0 <= nx < width:
                    working[ny, nx] = np.clip(working[ny, nx] + error * weight, 0.0, 1.0)
    return np.clip(output, 0.0, 1.0)


def _ordered_dither(image: np.ndarray, palette: np.ndarray, matrix: np.ndarray) -> np.ndarray:
    height, width, _ = image.shape
    mh, mw = matrix.shape
    tiled = np.tile(matrix, (int(np.ceil(height / mh)), int(np.ceil(width / mw))))[:height, :width]
    # Spread the ordered threshold across channels for gentle colour shifts.
    noise = (tiled - 0.5)[..., None]
    strength = 1.0 / max(len(palette) - 1, 1)
    adjusted = np.clip(image + noise * strength, 0.0, 1.0)
    flat = adjusted.reshape(-1, 3)
    indices = _nearest_palette_indices(flat, palette)
    quantised = palette[indices].reshape(image.shape)
    return np.clip(quantised, 0.0, 1.0)


def _random_dither(image: np.ndarray, palette: np.ndarray) -> np.ndarray:
    noise = (np.random.rand(*image.shape[:2], 1) - 0.5) * (1.0 / max(len(palette) - 1, 1))
    adjusted = np.clip(image + noise, 0.0, 1.0)
    flat = adjusted.reshape(-1, 3)
    indices = _nearest_palette_indices(flat, palette)
    return palette[indices].reshape(image.shape)


def _nearest_palette_indices(pixels: np.ndarray, palette: np.ndarray) -> np.ndarray:
    diff = pixels[:, None, :] - palette[None, :, :]
    distances = np.sum(diff * diff, axis=2)
    return np.argmin(distances, axis=1)


def _srgb_to_linear(rgb: np.ndarray) -> np.ndarray:
    threshold = 0.04045
    below = rgb <= threshold
    result = np.empty_like(rgb)
    result[below] = rgb[below] / 12.92
    result[~below] = ((rgb[~below] + 0.055) / 1.055) ** 2.4
    return result


def _linear_to_srgb(rgb: np.ndarray) -> np.ndarray:
    threshold = 0.0031308
    below = rgb <= threshold
    result = np.empty_like(rgb)
    result[below] = rgb[below] * 12.92
    result[~below] = 1.055 * np.power(rgb[~below], 1 / 2.4) - 0.055
    return np.clip(result, 0.0, 1.0)


def _adjust_gamma(image: np.ndarray, gamma: float) -> np.ndarray:
    """Apply gamma correction with an optional scikit-image fallback."""

    if exposure is not None:
        return exposure.adjust_gamma(image, gamma)
    gamma = max(gamma, 1e-6)
    return np.power(np.clip(image, 0.0, 1.0), gamma)


def _float_to_uint8(data: np.ndarray) -> np.ndarray:
    """Convert a float image in ``[0, 1]`` to ``uint8`` safely."""

    return np.clip(np.rint(data * 255.0), 0, 255).astype(np.uint8)


def _gaussian_blur(data: np.ndarray, radius: float) -> np.ndarray:
    """Apply a Gaussian blur using OpenCV when available, else Pillow."""

    sigma = max(1e-3, float(radius))
    if cv2 is not None:
        try:
            return cv2.GaussianBlur(data, ksize=(0, 0), sigmaX=sigma, sigmaY=sigma)
        except Exception:
            pass
    image = Image.fromarray(_float_to_uint8(data), mode="RGB")
    blurred = image.filter(ImageFilter.GaussianBlur(radius=sigma))
    return np.asarray(blurred, dtype=np.float32) / 255.0


def _unsharp_mask(data: np.ndarray, amount: float) -> np.ndarray:
    """Apply an unsharp mask using the best available backend."""

    amount = max(0.0, float(amount))
    if amount == 0.0:
        return data
    if cv2 is not None:
        try:
            sigma = 1.0
            blurred = cv2.GaussianBlur(data, ksize=(0, 0), sigmaX=sigma, sigmaY=sigma)
            return cv2.addWeighted(data, 1.0 + amount, blurred, -amount, 0)
        except Exception:
            pass
    image = Image.fromarray(_float_to_uint8(data), mode="RGB")
    percent = max(1, int(amount * 200))
    sharpened = image.filter(ImageFilter.UnsharpMask(radius=1.0, percent=percent, threshold=0))
    return np.asarray(sharpened, dtype=np.float32) / 255.0


def _denoise_image(data: np.ndarray, strength: float) -> np.ndarray:
    """Denoise the image, falling back to a light blur if OpenCV is missing."""

    strength = max(0.0, float(strength))
    if strength == 0.0:
        return data
    if cv2 is not None:
        try:
            temp = _float_to_uint8(data)
            h = 5 + int(strength * 10)
            denoised = cv2.fastNlMeansDenoisingColored(temp, None, h, h, 7, 21)
            return denoised.astype(np.float32) / 255.0
        except Exception:
            pass
    # As a conservative fallback, reuse a small Gaussian blur to reduce noise.
    return _gaussian_blur(data, radius=max(0.1, strength * 0.5))


__all__ = [
    "ImageProcessor",
    "DitheringSettings",
    "ProcessResult",
    "PALETTE_PRESETS",
]
