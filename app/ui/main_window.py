...
from app.core.dithering import (
    DEFAULT_PARAMETER_CONTROLS,
    algorithm_spec,
    parameter_control,
)
...
class MainWindow(QMainWindow):
    ...
    def _build_algorithm_controls(self) -> QWidget:
        ...
        self._amplitude_slider = self._make_parameter_slider("amplitude", self._queue_update)
        amplitude_control = DEFAULT_PARAMETER_CONTROLS["amplitude"]
        layout.addRow(amplitude_control.label, self._amplitude_slider)

        self._frequency_slider = self._make_parameter_slider("frequency", self._queue_update)
        frequency_control = DEFAULT_PARAMETER_CONTROLS["frequency"]
        layout.addRow(frequency_control.label, self._frequency_slider)

        self._period_slider = self._make_parameter_slider("period", self._queue_update)
        period_control = DEFAULT_PARAMETER_CONTROLS["period"]
        layout.addRow(period_control.label, self._period_slider)

        self._slope_slider = self._make_parameter_slider("slope", self._queue_update)
        slope_control = DEFAULT_PARAMETER_CONTROLS["slope"]
        layout.addRow(slope_control.label, self._slope_slider)

        self._rotation_slider = self._make_parameter_slider("rotation", self._queue_update)
        rotation_control = DEFAULT_PARAMETER_CONTROLS["rotation"]
        layout.addRow(rotation_control.label, self._rotation_slider)

        self._pixel_size_slider = self._make_slider(0, 100, 0, self._on_pixel_size_changed)
        layout.addRow("Pixel Size", self._pixel_size_slider)
        self._pixel_size_label = layout.labelForField(self._pixel_size_slider)
        self._update_pixel_size_label(self._pixel_size_slider.value())
        ...
        for name, widget in self._parameter_widgets.items():
            ...
            control = DEFAULT_PARAMETER_CONTROLS.get(name)
            if control and control.tooltip and label is not None:
    ...
    def _make_parameter_slider(self, name: str, slot: Callable[[int], None]) -> QSlider:
        control = DEFAULT_PARAMETER_CONTROLS[name]
        slider = self._make_slider(control.minimum, control.maximum, control.default, slot)
        slider.setSingleStep(control.step)
        if control.tooltip:
            slider.setToolTip(control.tooltip)
        return slider

    def _pixel_slider_to_size(self, value: int) -> int:
        normalised = max(0.0, min(1.0, value / 100.0))
        mapped = 1.0 + (normalised**2) * 63.0
        return max(1, int(round(mapped)))

    def _pixel_size_to_slider(self, size: int) -> int:
        clamped = max(1, min(size, 64))
        normalised = math.sqrt((clamped - 1) / 63.0) if clamped > 1 else 0.0
        return int(round(normalised * 100))

    def _update_pixel_size_label(self, slider_value: int) -> None:
        if hasattr(self, "_pixel_size_label") and self._pixel_size_label is not None:
            size = self._pixel_slider_to_size(slider_value)
    ...
    def _on_pixel_size_changed(self, value: int) -> None:
        self._update_pixel_size_label(value)
        self._queue_update()
    ...
    def _gather_settings(self) -> dict:
        return {
            ...
            "pixel_size": self._pixel_slider_to_size(self._pixel_size_slider.value()),
    ...
    def _apply_settings(self, settings: dict) -> None:
        ...
        if "pixel_size" in settings:
            slider_value = self._pixel_size_to_slider(settings["pixel_size"])
            self._pixel_size_slider.setValue(slider_value)
    ...
    def _on_algorithm_changed(self, algorithm: str) -> None:
        ...
        for name, widget in self._parameter_widgets.items():
            ...
            if name in active:
                control = parameter_control(name, spec)
                widget.blockSignals(True)
                widget.setRange(control.minimum, control.maximum)
                widget.setSingleStep(control.step)
                widget.setValue(control.default)
                widget.blockSignals(False)
                widget.setEnabled(True)
                widget.setToolTip(control.tooltip or "")
                if label is not None:
                    label.setEnabled(True)
                    label.setText(control.label)
                    label.setToolTip(control.tooltip or "")
            else:
