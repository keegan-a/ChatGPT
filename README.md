...
- Adjust threshold plus algorithm-specific parameters surfaced with descriptive names (e.g. Dot Pitch, Stripe Depth, Spiral Twist) that only appear when the selected shader makes use of them.
- Dial in block size with the tapered pixel-size control to down-sample before dithering for chunkier, faster renders when desired.
- Add luminous halos with a tone-aware glow that blooms highlights instead of simply blurring the entire frame.
