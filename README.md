...
2. **Replace the base64 icon sources** in the `icons/` folder if you want custom artwork. The repository ships text-based `.base64` files (and the SVG logo) so that no binary assets are tracked. The build tooling automatically decodes them into PNG/ICO/ICNS outputs under `dist/icons/` and `build/icons/` whenever you run `npm run prepare:web` or any packaging command. Drop in your own icons by regenerating the base64 text (or by setting `BUDGET95_SKIP_ICON_CHECK=1` and providing files manually) before preparing the build.
